    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm space-y-4">
        ...
        <p className="text-xs text-slate-500">
          O CUB nunca diminui. Se o valor informado for menor que o anterior, o
          sistema mantém o maior.
        </p>
        ...
        <form onSubmit={salvar} className="space-y-3">
          ...
          <div>
            ...
            <input
              type="number"
              value={form.valor_cub}
              onChange={(e) => setForm({ ...form, valor_cub: e.target.value })}
              step="0.01"
              placeholder="Ex: 3096.25"
              required
              autoFocus
              className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-400"
            />
...
function ModalParcela({
  ...
  const [form, setForm] = useState({
    ...
    valor_pago: calcularParcela(imovel.parcelas_cubs, cubEfetivoValor).toFixed(
      2,
    ),
    observacao: "",
    adiantada: false,
  ...
  function atualizarCub(val: string) {
    ...
    const valor = isNaN(cub)
      ? ""
      : calcularParcela(imovel.parcelas_cubs, cub).toFixed(2);
  ...
  async function salvar(e: React.FormEvent) {
    ...
    if (isNaN(numInicial) || numInicial < 1) {
      setErro("Número de parcela inválido.");
      return;
    }
    setSalvando(true);
    setErro(null);
    ...
    const { error } = await supabase
      .from("imovel_parcelas")
      .upsert(parcelas, { onConflict: "numero_parcela" });
    setSalvando(false);
    if (error) {
      setErro("Erro: " + error.message);
      return;
        ...
        <form onSubmit={salvar} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, numero_parcela: e.target.value })
                }
                min="1"
                max="100"
                required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="number"
                value={form.cub_usado}
                onChange={(e) => atualizarCub(e.target.value)}
                step="0.01"
                required
                className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-blue-400"
              />
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() =>
                setForm((f) => ({ ...f, adiantada: !f.adiantada }))
              }
            ...
            >
              <Zap size={12} />{" "}
              {form.adiantada ? "Parcela Adiantada ✓" : "Marcar como Adiantada"}
            </button>
            {form.adiantada && (
              <span className="text-[10px] text-amber-500">
                Será contabilizada como economia
              </span>
            )}
          </div>
...
function ModalReforco({
  ...
  const cubsRestantes = parseFloat(
    (CUBS_ESCRITURA - cubsPagosEscritura).toFixed(4),
  );
  const valorEscrituraHoje = parseFloat(
    (cubsRestantes * cubAtualValor).toFixed(2),
  );
  ...
  function atualizarReais(val: string) {
    ...
    if (isEscritura) {
      ...
      const cubs =
        cubAtualValor > 0 && !isNaN(reais)
          ? (reais / cubAtualValor).toFixed(4)
          : "";
  ...
  async function salvar(e: React.FormEvent) {
    ...
    if (isEscritura) {
      ...
      if (isNaN(cubs) || cubs <= 0) {
        setErro("Informe os CUBs pagos.");
        return;
      }
      if (cubs > cubsRestantes + 0.001) {
        setErro(`Máximo de CUBs restantes: ${cubsRestantes}`);
        return;
      }
    }
    ...
    const { error } = await supabase.from("imovel_reforcos").insert({
      descricao:
        form.descricao || (isEscritura ? "Pagamento Escritura" : "Reforço"),
      valor_reais: valor,
        ...
        <form onSubmit={salvar} className="space-y-3">
            ...
            <div className="p-4 bg-purple-50 border border-purple-200 rounded-xl space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <div>
                  ...
                  <div className="text-sm font-black text-emerald-600">
                    {cubsPagosEscritura.toFixed(4)} CUBs
                  </div>
                </div>
                <div>
              ...
              <div className="w-full bg-white rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (cubsPagosEscritura / CUBS_ESCRITURA) * 100)}%`,
                  }}
                />
              </div>
              <div className="text-xs text-purple-600 font-semibold text-center">
                Valor restante hoje:{" "}
                <span className="font-black">{fmt(valorEscrituraHoje)}</span>
              </div>
              {cubsRestantes > 0 && (
                <button
                  type="button"
                  onClick={preencherEscrituraTotal}
              ...
              {cubsRestantes <= 0 && (
                <div className="text-center text-emerald-600 font-bold text-xs">
                  ✓ Escritura totalmente quitada!
                </div>
              )}
            </div>
          )}
          ...
          <div>
            ...
            <input
              ...
              placeholder={
                isEscritura
                  ? "Ex: Pagamento Escritura (parcial)"
                  : "Ex: 3º Reforço da Reserva"
              }
              required
              autoFocus
              className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-400"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => atualizarReais(e.target.value)}
                step="0.01"
                placeholder="0,00"
                required
                className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-400"
            ...
            {isEscritura ? (
              <div>
                ...
                <input
                  type="number"
                  value={form.cubs_pagos}
                  onChange={(e) =>
                    setForm({ ...form, cubs_pagos: e.target.value })
                  }
                  step="0.0001"
                  placeholder={`Máx: ${cubsRestantes}`}
                  max={cubsRestantes}
            ...
            <tbody className="divide-y divide-slate-50">
              {sorted.map((c, i) => {
                ...
                return (
                  <tr
                    key={c.id}
                    className={`hover:bg-slate-50/70 ${isAtual ? "bg-amber-50/40" : ""}`}
                  >
                    <td className="py-3 pr-3 font-bold text-slate-700">
                      {c.mes_ano}
                    </td>
                    <td className="py-3 pr-3 text-right font-black text-slate-800">
                      {fmt(c.valor_cub)}
                    </td>
                    <td className="py-3 pr-3 text-right text-emerald-600 font-bold">
                      {fmt(valorParcela)}
                    </td>
                    <td className="py-3 pr-3 text-slate-400">
                      {fmtDate(c.data_registro)}
                    </td>
...
export default function Apartamento() {
  ...
  const [aba, setAba] = useState<"normais" | "adiantadas" | "reforcos" | "cub">(
    "normais",
  );
  ...
  const carregar = useCallback(async () => {
    setCarregando(true);
    const [{ data: im }, { data: cub }, { data: parc }, { data: ref }] =
      await Promise.all([
  ...
  const totalPagoNormais = parcelasNormais.reduce(
    (s, p) => s + p.valor_pago,
    0,
  );
  const totalPagoAdiantadas = parcelasAdiantadas.reduce(
    (s, p) => s + p.valor_pago,
    0,
  );
  ...
  const parcelasRestantes =
    imovel.total_parcelas - parcelasNormais.length - parcelasAdiantadas.length;
  const proximaParcelaValor = calcularParcela(
    imovel.parcelas_cubs,
    cubEfetivoValor,
  );
  ...
  const totalCubsOriginal =
    imovel.valor_original / imovel.cub_referencia_original;
  ...
  const cubsRestantesEscritura = parseFloat(
    (cubsEscrituraTotal - cubsPagosEscritura).toFixed(4),
  );
  const valorEscrituraAtual = cubsRestantesEscritura * cubEfetivoValor;
  const escrituraQuitada = cubsRestantesEscritura <= 0;
  const totalRestante =
    parcelasRestantes * proximaParcelaValor +
    (escrituraQuitada ? 0 : valorEscrituraAtual);

  const progresso = (totalPago / valorAtualizado) * 100;
  const mesesRestantes = mesesAte(imovel.data_entrega_chaves);
  const pctTempo = progressoTempo(
    imovel.data_inicio_parcelas,
    imovel.data_entrega_chaves,
  );
  ...
  const listaAtual = (listaAba as any[]).slice(
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-1">
          ...
          <div className="text-xs text-slate-400">
            ...
            <span className="text-slate-600 font-semibold">
              {fmt(imovel.valor_original)}
            </span>
            {" · "}
            <span className="text-emerald-600 font-bold">
              +{fmt(valorAtualizado - imovel.valor_original)}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-3">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-slate-400">
              Entrega prevista: {fmtDate(imovel.data_entrega_chaves)}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-3">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-slate-400">
              {progresso.toFixed(1)}% · de {fmt(valorAtualizado)}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="border-t border-slate-100 pt-2 flex justify-between">
              <span className="text-slate-500 text-xs">
                {parcelasNormais.length} parcelas normais:
              </span>
              <span className="font-bold text-slate-700 text-xs">
                {fmt(totalPagoNormais)}
              </span>
            </div>
            {parcelasAdiantadas.length > 0 && (
              <div className="flex justify-between">
                <span className="text-slate-500 text-xs">
                  {parcelasAdiantadas.length} adiantadas:
                </span>
                <span className="font-bold text-amber-600 text-xs">
                  {fmt(totalPagoAdiantadas)}
                </span>
              </div>
            )}
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-3">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-500 text-xs">
                {parcelasRestantes} parcelas × {fmt(proximaParcelaValor)}:
              </span>
              <span className="font-bold text-slate-700 text-xs">
                {fmt(parcelasRestantes * proximaParcelaValor)}
              </span>
            </div>
            <div className="flex justify-between">
              {escrituraQuitada ? (
                ...
                <span className="text-slate-500 text-xs">
                  Escritura ({cubsRestantesEscritura.toFixed(4)} CUBs
                  restantes):
                </span>
              )}
              <span
                className={`font-bold text-xs ${escrituraQuitada ? "text-emerald-600" : "text-slate-700"}`}
              >
            ...
            {!escrituraQuitada && (
              <div className="text-[10px] text-purple-600 bg-purple-50 rounded-lg px-2 py-1">
                {cubsPagosEscritura > 0 &&
                  `${cubsPagosEscritura.toFixed(4)} CUBs já pagos · `}
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 p-5 rounded-2xl text-white shadow-sm space-y-3">
          ...
          <div className="text-xs text-indigo-200">
            Parcela #{proximaParcela} · {fmtDate(dataProximaParcela)}
          </div>
          <div className="text-xs text-indigo-200">
            {imovel.parcelas_cubs} CUBs × {fmt(cubEfetivoValor)}
          </div>
          ...
          <div className="flex gap-1.5 flex-wrap">
            {(
              [
                {
                  key: "normais",
                  label: `Parcelas Normais (${parcelasNormais.length})`,
                },
                {
                  key: "adiantadas",
                  label: `Parcelas Adiantadas (${parcelasAdiantadas.length})`,
                },
        ...
        <div className="overflow-x-auto">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-slate-400 text-[10px] font-bold uppercase tracking-wider border-b border-slate-50">
                  ...
                  {aba === "adiantadas" && (
                    <th className="text-right px-4 py-3">Valor Hoje</th>
                  )}
                  {aba === "adiantadas" && (
              ...
              <tbody className="divide-y divide-slate-50">
                {(listaAtual as Parcela[]).map((p) => {
                  const valorHoje = calcularParcela(
                    imovel.parcelas_cubs,
                    cubEfetivoValor,
                  );
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-black ${p.adiantada ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                        >
              ...
              {aba === "adiantadas" && parcelasAdiantadas.length > 0 && (
                <tfoot className="border-t-2 border-slate-200 bg-slate-50">
                  <tr>
                    ...
                    >
                      Economia Total com Adiantamentos:
                    </td>
                    <td className="px-4 py-3 text-right">
          ...
          {aba === "reforcos" && (
            <table className="w-full text-sm">
                ...
                {(listaAtual as Reforco[]).map((r) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right text-xs text-slate-400">
                      {r.valor_cubs ? `${r.valor_cubs} CUBs` : "—"}
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-500">
              ...
              <tfoot className="border-t-2 border-slate-200 bg-slate-50">
                <tr>
                  <td className="px-5 py-3 text-xs font-bold text-slate-500 uppercase">
                    Total em Reforços:
                  </td>
                ...
                {(
                  ...
                  .map((c, i) => {
                    ...
                    return (
                      <tr
                        key={c.id}
                        className={`hover:bg-slate-50/60 transition-colors ${isAtual ? "bg-amber-50/50" : ""}`}
                      >
                        ...
                        <td className="px-4 py-3 text-right font-bold text-emerald-600">
                          {fmt(
                            calcularParcela(imovel.parcelas_cubs, c.valor_cub),
                          )}
                        </td>
                      </tr>
                    );
