  ...
  import {
    type CdiMap,
    buscarHistoricoCDI, diasCorridos, calcularJurosCompostos,
  } from '../lib/cdi'
  ...
  export default function Caixinhas({ onTotalCalculado }: { onTotalCalculado?: (total: number) => void }) {
    ...
    async function carregar() {
      ...
      const positivos = aportesCarregados.filter((a: Aporte) => a.valor_adicionado > 0)
      if (positivos.length === 0) return
      const maisAntigo = [...positivos].sort((a: Aporte, b: Aporte) => a.data_aporte.localeCompare(b.data_aporte))[0]
    ...
    // ── Callback para o App.tsx ──────────────────────────────────────────────
    useEffect(() => {
      if (!onTotalCalculado) return
      const total = totalDepositadoGlobal + totalRendimentoReal
      onTotalCalculado(total > 0 ? total : totalGuardado)
    }, [totalDepositadoGlobal, totalRendimentoReal, totalGuardado])
